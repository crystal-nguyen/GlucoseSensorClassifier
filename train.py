...
# load in data
insulin1 = pd.read_csv('InsulinData.csv', low_memory=False,usecols=['Index','Date','Time','BWZ Carb Input (grams)'])
insulin2 = pd.read_csv('InsulinAndMealIntake670GPatient3.csv', low_memory=False,usecols=['Index','Date','Time','BWZ Carb Input (grams)'])
cgm1 = pd.read_csv('CGMData.csv', low_memory=False, usecols=['Index','Date','Time','Sensor Glucose (mg/dL)'])
cgm2 = pd.read_csv('CGMData670GPatient3.csv',low_memory=False,usecols=['Index','Date','Time','Sensor Glucose (mg/dL)'])
...
meals1 = insulin1.loc[(insulin1['BWZ Carb Input (grams)'].isna()==False) & (insulin1['BWZ Carb Input (grams)']!=0)][['Index','DateTime']]
meals2 = insulin2.loc[(insulin2['BWZ Carb Input (grams)'].isna()==False) & (insulin2['BWZ Carb Input (grams)']!=0)][['Index','DateTime']]
...
for i in range(len(noMealStartTime1) - 2):
    ...
    noMeal_i = cgm1.iloc[postNoMealStartIndex:noMealStartIndex + 1, :]['Sensor Glucose (mg/dL)'].iloc[::-1].to_numpy()
...
for i in range(len(noMealStartTime2) - 2):
    ...
    noMealStartIndex = len(noMealStartTemp) - 1
    postNoMealStartIndex = noMealStartIndex - 23
    noMeal_i = cgm2.iloc[postNoMealStartIndex:noMealStartIndex + 1, :]['Sensor Glucose (mg/dL)'].iloc[::-1].to_numpy()
...
for i in range(len(noMealData)):
    ...
    # feature 5 - window mean
    window_mean.append((nomeal_window1.iloc[i].mean() + nomeal_window2.iloc[i].mean() + nomeal_window3.iloc[i].mean() +
...
X_train, X_test, y_train, y_test = train_test_split(featureMatrix[[1,2,3,4,5]], featureMatrix[['class']],test_size=0.33,random_state=42)
sc = StandardScaler()
...
pickle_out = open('clf_train.pickle','wb')
